...
def download(myinput, mydir = "./"):
    if isinstance(myinput, str) or isinstance(myinput, bytes):
        ...
        outfile = str(mydir) + str("/") + os.path.basename(urlparse(str(myinput)).path)
        outfile = outfile[:-1] # remove ' at the end
        #print(outfile)
...
class Pinterest_Helper(object):
    ...
    def __init__(self, login, pw, browser = None):
        ...
        #connect = self.browser.find_element_by_class_name('GoogleConnectButton active')
        #connect.click()
        randdelay(2,4)
        #goog = self.browser.find_elements_by_xpath('//*[@id="googleConnectButton"]')
        #goog.click()
        #randdelay(2,4)
