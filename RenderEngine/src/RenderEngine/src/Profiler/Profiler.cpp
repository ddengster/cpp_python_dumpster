...
Profiler::~Profiler( void )
{
	...
	endClock = clock();
	uint32_t clockDiff = endClock - startClock;

	Real timeDiff = clockDiff * SEC_PER_TICK;
...
String ProfileData::WriteAsString( const String& profileName )
{
	...
	//ss.setf(ss.fixed);
	//ss.precision(2);
	ss << "  " << ceil((timeTaken/(Real)instancesCount)*1000) << "\t\t|"		//Average Time per iteration
