...
//-----------------------------------------------------------------------
Vector3 Matrix3::GetColumn(uint32_t iCol) const
{
...
//-----------------------------------------------------------------------
void Matrix3::SetColumn(uint32_t iCol, const Vector3& vec)
{
    assert( 0 <= iCol && iCol < 3 );
    m[0][iCol] = vec.x;
    m[1][iCol] = vec.y;
...
//-----------------------------------------------------------------------
bool Matrix3::IsEquals(const Matrix3& rkMatrix) const
{
    for (uint32_t iRow = 0; iRow < 3; iRow++)
    {
        for (uint32_t iCol = 0; iCol < 3; iCol++)
...
void Matrix3::Add(Matrix3& result, const Matrix3& leftOperandMatrix, const Matrix3& rightOperandMatrix)
{
    for (uint32_t iRow = 0; iRow < 3; iRow++)
    {
        for (uint32_t iCol = 0; iCol < 3; iCol++)
        {
            result.m[iRow][iCol] = leftOperandMatrix.m[iRow][iCol] +
								rightOperandMatrix.m[iRow][iCol];
        }
    }
}
//-----------------------------------------------------------------------
...
//-----------------------------------------------------------------------
void Matrix3::Subtract(Matrix3& result, const Matrix3& leftOperandMatrix, const Matrix3& rightOperandMatrix)
{
    for (uint32_t iRow = 0; iRow < 3; iRow++)
    {
        for (uint32_t iCol = 0; iCol < 3; iCol++)
        {
            result.m[iRow][iCol] = leftOperandMatrix[iRow][iCol] -
                rightOperandMatrix.m[iRow][iCol];
        }
    }
}
...
Vector3 Matrix3::operator*(const Vector3& rkPoint) const
{
 ...
 //-----------------------------------------------------------------------
void Matrix3::Multiply(Vector3& result, const Matrix3& rkMatrix, const Vector3& rkVector)
{
    for (uint32_t iRow = 0; iRow < 3; iRow++)
    {
        result[iRow] =
            rkMatrix[iRow][0]*rkVector[0] +
            rkMatrix[iRow][1]*rkVector[1] +
            rkMatrix[iRow][2]*rkVector[2];
    }
}
//-----------------------------------------------------------------------
...
//-----------------------------------------------------------------------
void Multiply(Vector3& result, const Vector3& rkVector, const Matrix3& rkMatrix)
{
    for (uint32_t iRow = 0; iRow < 3; iRow++)
    {
        result[iRow] =
...
//-----------------------------------------------------------------------
void Matrix3::Negation(Matrix3& result, const Matrix3& rkMatrix)
{
    for (uint32_t iRow = 0; iRow < 3; iRow++)
    {
        for (uint32_t iCol = 0; iCol < 3; iCol++)
            result[iRow][iCol] = -rkMatrix.m[iRow][iCol];
    }
}
//-----------------------------------------------------------------------
...
//-----------------------------------------------------------------------
void Multiply(Matrix3& result, const Matrix3& rkMatrix, Real fScalar)
{
    for (uint32_t iRow = 0; iRow < 3; iRow++)
    {
        for (uint32_t iCol = 0; iCol < 3; iCol++)
            result[iRow][iCol] = fScalar*rkMatrix.m[iRow][iCol];
    }
}
//-----------------------------------------------------------------------
...
//-----------------------------------------------------------------------
void Multiply(Matrix3& result, Real fScalar, const Matrix3& rkMatrix)
{
    for (uint32_t iRow = 0; iRow < 3; iRow++)
    {
        for (uint32_t iCol = 0; iCol < 3; iCol++)
...
bool Matrix3::Inverse(Matrix3& rkInverse, Real fTolerance) const
{
    ...
    Real fDet = m[0][0]*rkInverse[0][0] + m[0][1]*rkInverse[1][0]+ m[0][2]*rkInverse[2][0];

    if (GenericMath::FAbs(fDet) <= fTolerance)
        return false;
...
Real Matrix3::Determinant() const
{
    ...
    Real fDet = m[0][0]*fCofactor00 + m[0][1]*fCofactor10 + m[0][2]*fCofactor20;
...
void Matrix3::Bidiagonalize(Matrix3& kA, Matrix3& kL, Matrix3& kR)
{
    ...
    fLength = GenericMath::Sqrt(kA[0][0]*kA[0][0] + kA[1][0]*kA[1][0] + kA[2][0]*kA[2][0]);
    if (fLength > 0.0)
    {
    ...
    // map first row to (*,*,0)
    fLength = GenericMath::Sqrt(kA[0][1]*kA[0][1]+kA[0][2]*kA[0][2]);
    if (fLength > 0.0)
    {
    ...
    fLength = GenericMath::Sqrt(kA[1][1]*kA[1][1]+kA[2][1]*kA[2][1]);
    if (fLength > 0.0)
    {
...
void Matrix3::GolubKahanStep(Matrix3& kA, Matrix3& kL, Matrix3& kR)
{
    ...
    Real fDiscr = GenericMath::Sqrt(fDiff*fDiff+4.0*fT12*fT12);
    Real fRoot1 = 0.5*(fTrace+fDiscr);
    Real fRoot2 = 0.5*(fTrace-fDiscr);

    // adjust right
    Real fY = kA[0][0] - (GenericMath::FAbs(fRoot1-fT22) <= GenericMath::FAbs(fRoot2-fT22) ? fRoot1 : fRoot2);
    Real fZ = kA[0][1];
	Real fInvLength = GenericMath::InvSqrt(fY*fY+fZ*fZ);
    ...
    fY = kA[0][0];
    fZ = kA[1][0];
    fInvLength = GenericMath::InvSqrt(fY*fY+fZ*fZ);
    fSin = fZ*fInvLength;
    fCos = -fY*fInvLength;
...
void Matrix3::SingularValueDecomposition(Matrix3& kL, Vector3& kS, Matrix3& kR) const
{
    ...
    for (unsigned int i = 0; i < ms_iSvdMaxIterations; i++)
    {
        ...
        bool bTest1 = (GenericMath::FAbs(kA[0][1]) <= ms_fSvdEpsilon*(GenericMath::FAbs(kA[0][0])+GenericMath::FAbs(kA[1][1])));
        bool bTest2 = (GenericMath::FAbs(kA[1][2]) <= ms_fSvdEpsilon*(GenericMath::FAbs(kA[1][1])+GenericMath::FAbs(kA[2][2])));
        if (bTest1)
        {
            if (bTest2)
            {
            ...
            else
            {
                ...
                fTan0 = 0.5*(fTmp+GenericMath::Sqrt(fTmp*fTmp + 4.0));
                fCos0 = GenericMath::InvSqrt(1.0+fTan0*fTan0);
                fSin0 = fTan0*fCos0;
                ...
                fCos1 = GenericMath::InvSqrt(1.0+fTan1*fTan1);
                fSin1 = -fTan1*fCos1;
...
void Matrix3::OrthoNormalize()
{
    ...
    // compute q0
    Real fInvLength = GenericMath::InvSqrt(m[0][0]*m[0][0] + m[1][0]*m[1][0] + m[2][0]*m[2][0]);
    ...
    // compute q1
    Real fDot0 = m[0][0]*m[0][1] + m[1][0]*m[1][1] + m[2][0]*m[2][1];

    m[0][1] -= fDot0*m[0][0];
    m[1][1] -= fDot0*m[1][0];
    m[2][1] -= fDot0*m[2][0];

    fInvLength = GenericMath::InvSqrt(m[0][1]*m[0][1] + m[1][1]*m[1][1] + m[2][1]*m[2][1]);
    ...
    // compute q2
    Real fDot1 = m[0][1]*m[0][2] +  m[1][1]*m[1][2] +  m[2][1]*m[2][2];
    ...
    fInvLength = GenericMath::InvSqrt(	m[0][2]*m[0][2] +
										m[1][2]*m[1][2] +
										m[2][2]*m[2][2]);
    ...
    Vector3& kD, Vector3& kU) const
{
    ...
    Real fInvLength = GenericMath::InvSqrt(	m[0][0]*m[0][0] + 
											m[1][0]*m[1][0] +
											m[2][0]*m[2][0]);
    ...
    fInvLength = GenericMath::InvSqrt(kQ[0][1]*kQ[0][1] + kQ[1][1]*kQ[1][1] + kQ[2][1]*kQ[2][1]);
    kQ[0][1] *= fInvLength;
    kQ[1][1] *= fInvLength;
    kQ[2][1] *= fInvLength;
    ...
    kQ[1][2] -= fDot*kQ[1][1];
    kQ[2][2] -= fDot*kQ[2][1];
    fInvLength = GenericMath::InvSqrt(	kQ[0][2]*kQ[0][2] +
										kQ[1][2]*kQ[1][2] +
										kQ[2][2]*kQ[2][2]);
    kQ[0][2] *= fInvLength;
    kQ[1][2] *= fInvLength;
...
Real Matrix3::MaxCubicRoot(Real afCoeff[3])
{
    ...
    if (fPoly < 0.0)
    {
        ...
        fX = GenericMath::FAbs(afCoeff[0]);
        Real fTmp = 1.0+GenericMath::FAbs(afCoeff[1]);
        if (fTmp > fX)
            fX = fTmp;
        fTmp = 1.0+GenericMath::FAbs(afCoeff[2]);
        if (fTmp > fX)
            fX = fTmp;
    }
    ...
    for (int i = 0; i < 16; i++)
    {
        ...
        if (GenericMath::FAbs(fPoly) <= fEpsilon)
            return fX;
...
Real Matrix3::SpectralNorm() const
{
    ...
    Real fNorm = GenericMath::Sqrt(fPmax*fRoot);
    return fNorm;
}
//-----------------------------------------------------------------------
void Matrix3::ToAxisAngle(Vector3& rkAxis, Radian& rfRadians) const
{
	...
	rfRadians.SetValue(GenericMath::ACos(fCos));  // in [0,PI]
        ...
        else
        {
            ...
            if ( m[0][0] >= m[1][1] )
            {
                ...
                if (m[0][0] >= m[2][2])
                {
                    ...
                    rkAxis.x = 0.5*GenericMath::Sqrt(m[0][0] - m[1][1] - m[2][2] + 1.0);
                ...
                else
                {
                    ...
                    rkAxis.z = 0.5*GenericMath::Sqrt(m[2][2] - m[0][0] - m[1][1] + 1.0);
            ...
            else
            {
                ...
                if (m[1][1] >= m[2][2])
                {
                    ...
                    rkAxis.y = 0.5*GenericMath::Sqrt(m[1][1] - m[0][0] - m[2][2] + 1.0);
...
{
	Real fCos = GenericMath::Cos(fRadians.GetValue());
    Real fSin = GenericMath::Sin(fRadians.GetValue());
    ...
    Radian& rfRAngle) const
{
    ...
    rfPAngle = Radian(GenericMath::ASin(m[0][2]));
	if (rfPAngle < Radian(GenericMath::HALF_PI))
    {
        if (rfPAngle > Radian(-GenericMath::HALF_PI))
        {
			rfYAngle.SetValue(GenericMath::ATan2(-m[1][2],m[2][2]));
            rfRAngle.SetValue(GenericMath::ATan2(-m[0][1],m[0][0]));
            return true;
        }
        else
        {
            // WARNING.  Not a unique solution.
            Radian fRmY(GenericMath::ATan2(m[1][0],m[1][1]));
            rfRAngle = Radian(0.0);  // any angle works
			rfYAngle.SetValue(rfRAngle.GetValue() - fRmY.GetValue());
            return false;
        }
    }
    else
    {
        // WARNING.  Not a unique solution.
        Radian fRpY(GenericMath::ATan2(m[1][0],m[1][1]));
        rfRAngle = Radian(0.0);  // any angle works
        rfYAngle.SetValue(fRpY.GetValue() - rfRAngle.GetValue());
        return false;
    }
}
//-----------------------------------------------------------------------
bool Matrix3::ToEulerAnglesXZY(Radian& rfYAngle, Radian& rfPAngle, Radian& rfRAngle) const
{
    ...
    rfPAngle.SetValue(GenericMath::ASin(-m[0][1]));
    if (rfPAngle < Radian(GenericMath::HALF_PI))
    {
        if (rfPAngle > Radian(-GenericMath::HALF_PI))
        {
            rfYAngle.SetValue(GenericMath::ATan2(m[2][1],m[1][1]));
            rfRAngle.SetValue(GenericMath::ATan2(m[0][2],m[0][0]));
            return true;
        }
        else
        {
            // WARNING.  Not a unique solution.
            Radian fRmY(GenericMath::ATan2(-m[2][0],m[2][2]));
            rfRAngle = Radian(0.0);  // any angle works
            rfYAngle.SetValue(rfRAngle.GetValue() - fRmY.GetValue());
            return false;
        }
    ...
    {
        // WARNING.  Not a unique solution.
        Radian fRpY(GenericMath::ATan2(-m[2][0],m[2][2]));
        rfRAngle = Radian(0.0);  // any angle works
        rfYAngle.SetValue(fRpY.GetValue() - rfRAngle.GetValue());
        return false;
    }
...
bool Matrix3::ToEulerAnglesYXZ(Radian& rfYAngle, Radian& rfPAngle, Radian& rfRAngle) const
{
	...
	rfPAngle.SetValue(GenericMath::ASin(-m[1][2]));
    if (rfPAngle < Radian(GenericMath::HALF_PI))
    {
        if (rfPAngle > Radian(-GenericMath::HALF_PI))
        {
            rfYAngle.SetValue(GenericMath::ATan2(m[0][2],m[2][2]));
            rfRAngle.SetValue(GenericMath::ATan2(m[1][0],m[1][1]));
            return true;
        }
        else
        {
            // WARNING.  Not a unique solution.
            Radian fRmY(GenericMath::ATan2(-m[0][1],m[0][0]));
            rfRAngle = Radian(0.0);  // any angle works
            rfYAngle.SetValue(rfRAngle.GetValue() - fRmY.GetValue());
            return false;
        }
    ...
    {
        // WARNING.  Not a unique solution.
        Radian fRpY(GenericMath::ATan2(-m[0][1],m[0][0]));
        rfRAngle = Radian(0.0);  // any angle works
...
bool Matrix3::ToEulerAnglesYZX(Radian& rfYAngle, Radian& rfPAngle, Radian& rfRAngle) const
{
    ...
    rfPAngle.SetValue(GenericMath::ASin(m[1][0]));
    if (rfPAngle < Radian(GenericMath::HALF_PI))
    {
        if (rfPAngle > Radian(-GenericMath::HALF_PI))
        {
			rfYAngle.SetValue(GenericMath::ATan2(-m[2][0],m[0][0]));
			rfRAngle.SetValue(GenericMath::ATan2(-m[1][2],m[1][1]));
            return true;
        }
        else
        {
            // WARNING.  Not a unique solution.
            Radian fRmY(GenericMath::ATan2(m[2][1],m[2][2]));
            rfRAngle = Radian(0.0);  // any angle works
            rfYAngle.SetValue(rfRAngle.GetValue() - fRmY.GetValue());
            return false;
        }
    ...
    {
        // WARNING.  Not a unique solution.
        Radian fRpY(GenericMath::ATan2(m[2][1],m[2][2]));
        rfRAngle = Radian(0.0);  // any angle works
        rfYAngle.SetValue(fRpY.GetValue() - rfRAngle.GetValue());
        return false;
    }
...
bool Matrix3::ToEulerAnglesZXY(Radian& rfYAngle, Radian& rfPAngle, Radian& rfRAngle) const
{
    ...
    rfPAngle.SetValue(GenericMath::ASin(m[2][1]));
    if (rfPAngle < Radian(GenericMath::HALF_PI))
    {
        if (rfPAngle > Radian(-GenericMath::HALF_PI))
        {
            rfYAngle.SetValue(GenericMath::ATan2(-m[0][1],m[1][1]));
            rfRAngle.SetValue(GenericMath::ATan2(-m[2][0],m[2][2]));
            return true;
        }
        else
        {
            // WARNING.  Not a unique solution.
            Radian fRmY(GenericMath::ATan2(m[0][2],m[0][0]));
            rfRAngle = Radian(0.0);  // any angle works
            rfYAngle.SetValue(rfRAngle.GetValue() - fRmY.GetValue());
            return false;
        }
    ...
    {
        // WARNING.  Not a unique solution.
        Radian fRpY(GenericMath::ATan2(m[0][2],m[0][0]));
        rfRAngle = Radian(0.0);  // any angle works
        rfYAngle.SetValue(fRpY.GetValue() - rfRAngle.GetValue());
        return false;
    }
...
bool Matrix3::ToEulerAnglesZYX(Radian& rfYAngle, Radian& rfPAngle, Radian& rfRAngle) const
{
    ...
    rfPAngle.SetValue(GenericMath::ASin(-m[2][0]));
    if (rfPAngle < Radian(GenericMath::HALF_PI))
    {
        if (rfPAngle > Radian(-GenericMath::HALF_PI))
        {
            rfYAngle.SetValue(GenericMath::ATan2(m[1][0],m[0][0]));
            rfRAngle.SetValue(GenericMath::ATan2(m[2][1],m[2][2]));
            return true;
        }
        else
        {
            // WARNING.  Not a unique solution.
            Radian fRmY(GenericMath::ATan2(-m[0][1],m[0][2]));
            rfRAngle = Radian(0.0);  // any angle works
            rfYAngle.SetValue(rfRAngle.GetValue() - fRmY.GetValue());
            return false;
        }
    ...
    {
        // WARNING.  Not a unique solution.
        Radian fRpY(GenericMath::ATan2(-m[0][1],m[0][2]));
        rfRAngle = Radian(0.0);  // any angle works
        rfYAngle.SetValue(fRpY.GetValue() - rfRAngle.GetValue());
        return false;
    }
...
void Matrix3::FromEulerAnglesXYZ(const Radian& fYAngle, const Radian& fPAngle, const Radian& fRAngle)
{
	...
	fCos = GenericMath::Cos(fYAngle.GetValue());
    fSin = GenericMath::Sin(fYAngle.GetValue());
    Matrix3 kXMat(1.0,0.0,0.0,0.0,fCos,-fSin,0.0,fSin,fCos);

    fCos = GenericMath::Cos(fPAngle.GetValue());
    fSin = GenericMath::Sin(fPAngle.GetValue());
    Matrix3 kYMat(fCos,0.0,fSin,0.0,1.0,0.0,-fSin,0.0,fCos);

    fCos = GenericMath::Cos(fRAngle.GetValue());
    fSin = GenericMath::Sin(fRAngle.GetValue());
...
void Matrix3::Tridiagonal(Real afDiag[3], Real afSubDiag[3])
{
    ...
    afDiag[0] = fA;
    afSubDiag[2] = 0.0;
    if ( GenericMath::FAbs(fC) >= EPSILON )
    {
        Real fLength = GenericMath::Sqrt(fB*fB+fC*fC);
...
bool Matrix3::QLAlgorithm(Real afDiag[3], Real afSubDiag[3])
{
    ...
    for (int i0 = 0; i0 < 3; i0++)
    {
        ...
        for (iIter = 0; iIter < iMaxIter; iIter++)
        {
            ...
            for (i1 = i0; i1 <= 1; i1++)
            {
                Real fSum = GenericMath::FAbs(afDiag[i1]) +
                    GenericMath::FAbs(afDiag[i1+1]);
                if ( GenericMath::FAbs(afSubDiag[i1]) + fSum == fSum )
                    break;
            }
            if ( i1 == i0 )
                break;
            ...
            Real fTmp1 = GenericMath::Sqrt(fTmp0*fTmp0+1.0);
            if ( fTmp0 < 0.0 )
            ...
            for (int i2 = i1-1; i2 >= i0; i2--)
            {
                ...
                if ( GenericMath::FAbs(fTmp3) >= GenericMath::FAbs(fTmp0) )
                {
                    fCos = fTmp0/fTmp3;
                    fTmp1 = GenericMath::Sqrt(fCos*fCos+1.0);
                ...
                {
                    fSin = fTmp3/fTmp0;
                    fTmp1 = GenericMath::Sqrt(fSin*fSin+1.0);
